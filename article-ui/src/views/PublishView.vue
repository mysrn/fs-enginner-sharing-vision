        ...
        <tr v-for="(post, index) in paginate(all_post)" 
          ...
          <td>
            ...
            <a class="badge bg-warning" 
              @click="changeToDraft(post.id)">
              <i class="bi bi-book"></i> draft</a>
    ...
    methods: {
      ...
      changeToDraft(id){
        ...
        if (confirm("Apakah Anda yakin ingin memindahkan ini ke tempat sampah?")) {
          axios.post('http://127.0.0.1:8000/api/article-draft/', data)
          .then((ress) => document.location="/published")
          .catch((err) => console.log(err))
        }
      },
      changeTotrash(id){
