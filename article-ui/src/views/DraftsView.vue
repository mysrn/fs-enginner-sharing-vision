        ...
        <tr v-for="(post, index) in paginate(all_post)" 
          ...
          <td>
            ...
            <a class="badge bg-success" 
              @click="changeToPublish(post.id)">
              <i class="bi bi-arrow-up"></i> publish</a>
            <a class="badge bg-danger" 
              @click="changeTotrash(post.id)">
    ...
    methods: {
      allPosts(){
        ...
        .then((ress) => {this.all_post = 
          ress.data.data.filter((post) => post.Status === 'Draft')
        })
      ...
      changeToPublish(id){
        const data = {id}
        if (confirm("Apakah Anda yakin ingin memindahkan ini ke tempat sampah?")) {
          axios.post('http://127.0.0.1:8000/api/article-publish/', data)
          .then((ress) => document.location="/drafts")
          .catch((err) => console.log(err))
        }
      },
      changeTotrash(id){
        const data = {id}
        if (confirm("Apakah Anda yakin ingin memindahkan ini ke tempat sampah?")) {
          axios.post('http://127.0.0.1:8000/api/article-trash/', data)
          .then((ress) => document.location="/drafts")
          .catch((err) => console.log(err))
        }
      },
